<div class="bg-light pb-3">
      <div class="container custom mb-5">
        <div class="row justify-content-center">
          <div class="col-md-6">
            <div class="title mt-5">CONNECT WITH US</div>
          </div>
        </div>
        <div class="row justify-content-center">
          <div class="col-md-9 text-center">
            <div class="content">
              Contact the way that satisfies you. Find us on the most popular
              social channels in world and let us help you arrange a plan for
              your upcoming itinerary.
            </div>
          </div>
        </div>
        <div class="row justify-content-center my-5">
          <div class="col-md-12">
            <div class="socials">
              <a href="https://www.facebook.com/Union-Tour-Operator-107489900797347/" target="_blank"
                ><span class="fa fa-facebook-square"></span> Facebook</a
              >
              <a href="https://www.instagram.com/uniontouroperator/" target="_blank"><span class="fa fa-instagram"></span> Instagram</a>
              <a href="mailto:infocust@uniontouroperator.com?subject=Hello Union Tour Operator"><span class="fa fa-envelope"></span> Email</a>
              <a target="_blank" href="https://wa.me/15551234567"><span class="fa fa-whatsapp"></span> Whatsapp</a>
            </div>
          </div>
        </div>
      </div>
    </div>

<div class="bg-footer">
      <div class="container custom">
        <div class="row my-5">
          <div class="col-md-3">
            <img src="<?php echo base_url() ?>assets/images/common/UNION.png" alt="" width="40%">
            <div class="link-footer mt-3">
                <b>Jl. Khairil Anwar No. 102 A kelurahan Badean Bondowoso 
68214 <a href="https://goo.gl/maps/oeBJq1398RhBGzF89" target="_blank" class="link-footer color-blue">View on map</a> </b>
            </div>
          </div>
          <div class="col-md-3">
            <div class="blue-title color-white">
              About union
            </div>
            <div class="link-footer">
                <b>Inspired by the life-altering vistas, charming cultures, and vibrant cities, Union`s vision has always been to empower others to discover the same 'magic of Indonesia' that our journey some two decades ago. </b>
            </div>
          </div>
          <div class="col-md-3">
            <div class="blue-title color-white">
              Site Link
            </div>
            <div class="row">
              <div class="col-md-6">
                <ul class="link-footer">
                  <li>
                    <a href="<?php echo base_url() ?>" class="color-blue">HOME</a>
                  </li>
                  <li>
                    <a href="<?php echo base_url()."packages" ?>" class="color-blue">ALL PACKAGES</a>
                  </li>
                  <li>
                    <a href="<?php echo base_url()."packages?id_style=1" ?>" class="color-blue">ACTIVE TRAVEL</a>
                  </li>
                  <li>
                    <a href="<?php echo base_url()."packages?id_style=2" ?>" class="color-blue"
                      >CLASSIC JOURNEY              </li>
                </ul>
              </div>
              <div class="col-md-6">
                <ul class="link-footer">
                  <li>
                    <a href="<?php echo base_url()."packages?id_style=3" ?>" class="color-blue">TREKKING</a>
                  </li>
                  <li>
                    <a href="<?php echo base_url()."about" ?>" class="color-blue">ABOUT</a>
                  </li>
                  <li>
                    <a href="#" data-toggle="modal" data-target="#myModal" class="color-blue">ENQUIRE</a>
                  </li>
                </ul>
              </div>
            </div>
          </div>
          <div class="col-md-3">
            <div class="blue-title color-white">
              Connect with us
            </div>
            <label for="" class="color-white">Subscribe our newsletter</label>
            <form action="">
              <input
                type="text"
                class="form-control"
                placeholder="Your email address"
              />
              <button class="btn btn-primary mt-2 btn-block">Subscribe</button>
            </form>
          </div>
        </div>
      </div>
    </div>
    <div class="container">
      <div class="row my-4">
        <div class="col-md-12 text-center">
          <div class="copyright">
            <span class="color-blue">Copyright 2020</span> - Union Tour Operator
          </div>
        </div>
      </div>
    </div>
    <script src="<?php echo base_url() ?>assets/js/jquery.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>

    <script src="<?php echo base_url() ?>assets/js/bootstrap.min.js"></script>
    <script src="<?php echo base_url() ?>assets/js/owl.carousel.min.js"></script>
    <script src="<?php echo base_url() ?>assets/js/select2-develop/dist/js/select2.min.js"></script>
    <script src='https://www.google.com/recaptcha/api.js'></script>
    <script src="<?php echo base_url() ?>assets/js/custom.js"></script>
  </body>
</html>
